...
| Kiosk silent for prefix-military (phones TTS OK) | Live SPA MP3 still DB-mil only; TTS skipped on kiosk; deliver-only Pi chime misses cooldown visits | planes-api `playKioskAlertSound` on **in-radius** military/special (`military-in-range`), not only after Pushover send; SPA `plane.isMilitary` still needed on next SPA ship |
