...
- **Agent shell wait:** use `block_until_ms` **141128** (~141s) — poll every 15s; do not pad to 15+ min upfront.
...
Updated: 2026-07-18T08:02:20Z · source: `directory/data/deploy-timing.json`
...
| Kiosk silent alerts (speaker OK; desktop browsers work) | Usually autoplay/TTS unlock or no Pi speech voices — **not** mute if UI shows `volume_up`. Mute only blocks mil/special MP3+TTS; icon had no `.active` highlight so state was easy to miss | Console `testAlertSound()` / `testTTS()`; `--autoplay-policy=no-user-gesture-required`; mute uses highlighted `volume_off` when on; kiosk still forces unmute on boot |
