...
- **2026-07-18** — Kiosk silent while phones TTS for in-range mil: do **not** trust `/v2/point` alone. Near Berlin it returns ~20–30 nearest and drops mil at 60–90km that phones still show when their map center differs from the home pin. Journal: `kiosk-chime` refetches + `militaryFlagged:0` while other snapshots still had GAF/RCH. Merge `/v2/mil` filtered to radius into `chimeKioskForMilitaryInRange`. Also `patchAdminFirestoreNamespace` must `Object.defineProperty` FieldValue/Timestamp — plain assign is a no-op, so `serverTimestamp()` stays a Sentinel → JSON `{}` → perpetual `snapshotAgeMs:null`.
...
| Kiosk silent; phones TTS; `militaryFlagged:0` but mil within radius | `/v2/point` capped near dense hubs; home-centered Pi fetch misses mil phones see on a different map center; `timestamp:{}` from unpatched FieldValue | Merge `/v2/mil` into kiosk chime; `defineProperty` FieldValue/Timestamp in `patchAdminFirestoreNamespace` |
