...
| Kiosk silent for prefix-military (phones TTS OK) | Live SPA MP3 still DB-mil only; TTS skipped on kiosk | planes-api plays local MP3 on prefix-only Pushover (`playKioskAlertSound`); SPA `plane.isMilitary` still needed on next SPA ship |
