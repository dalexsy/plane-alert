...
- **2026-08-05** — Ghost military TTS with “no tab open”: do **not** blame a hidden Chrome tab. Installed **Chrome App PlaneAlert** (`PlaneAlert.lnk`, app-id `oiojfjanafclfaakpadhhjebhfiodfma`) is a separate window — not in the tab strip. Do **not** gate TTS on `visibilityState` alone (other desktop / behind apps stays visible). Require **document.hasFocus()** for non-kiosk; cancel on blur. Do **not** strip TTS entirely — fix the focus gate + uninstall the PWA if unwanted.
