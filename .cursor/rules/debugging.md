...
| Kiosk silent MP3 alerts (Jabra OK; system sounds work) | Web Audio `createMediaElementSource` stayed silent on kiosk Chromium; TTS not used on kiosk | Plain `HTMLAudioElement.play()` + silent unlock; `--autoplay-policy=no-user-gesture-required`; console `testAlertSound()` |
