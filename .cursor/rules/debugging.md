...
- **Agent shell wait:** use `block_until_ms` **119844** (~120s) — poll every 15s; do not pad to 15+ min upfront.
...
Updated: 2026-07-17T14:01:50Z · source: `directory/data/deploy-timing.json`
...
| TTS/MP3 while PWA “closed” / not visible | `speechSynthesis` needs a live document — leftover background tab/minimized PWA kept scanning; no visibility gate. SW cannot TTS (FCM only shows OS toast) | `isPageAudible()` gates TTS + alert MP3; cancel speech on `visibilitychange` hidden |
