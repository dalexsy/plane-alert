...
| Kiosk silent for prefix-military (phones TTS OK) | Live SPA MP3 still DB-mil only; TTS skipped on kiosk; deliver-only Pi chime misses cooldown visits; detached `pw-play` may spawn with no Jabra stream | planes-api `playKioskAlertSound` on **in-radius** mil/special + after Pushover; **non-detached** spawn; SPA `plane.isMilitary` still needed on next SPA ship |
