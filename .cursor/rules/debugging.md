...
| Kiosk silent for prefix-military (phones TTS OK) | Live SPA MP3 still DB-mil only; TTS skipped on kiosk | planes-api plays local MP3 on **every** military/special Pushover (`playKioskAlertSound`); do not use prefix-only gate (dbFlags planes never chimed); SPA `plane.isMilitary` still needed on next SPA ship |
