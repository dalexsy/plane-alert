...
| Kiosk sounds with nothing military/special on SPA list | Pi PipeWire `military-in-range` chime (not SPA); ADS-B mil/dbFlags alone ≠ SPA isMilitary; phone radius ≠ kiosk view | Gate Pi chime on aircraftDb mil \| prefix \| special only; edge-trigger newly in-range (persist state); journal `Kiosk alert sound started` / `Kiosk chime newly in range` |
