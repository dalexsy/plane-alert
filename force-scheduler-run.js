...
async function getAccessToken() {
  try {
    ...
    try {
      ...
      throw new Error(
        "Unable to get access token. Make sure you are logged in to Firebase or gcloud.",
      );
    }
  }
}
      ...
      (res) => {
        ...
        res.on("end", () => {
          if (res.statusCode === 200) {
            ...
            console.error(
              `❌ Failed to trigger ${functionName}: ${res.statusCode}`,
            );
            console.error(data);
...
async function main() {
  ...
  console.log(
    "Note: You can also manually trigger these in the Firebase Console:",
  );
  console.log(
    "https://console.cloud.google.com/cloudscheduler?project=plane-alert-800ff",
  );
  console.log("\nAlternatively, check the logs here:");
  console.log(
    "https://console.cloud.google.com/logs/query;query=resource.type%3D%22cloud_function%22%0Aresource.labels.function_name%3D%22collectAircraftData%22?project=plane-alert-800ff",
  );
  ...
  const checkInterval = setInterval(async () => {
    const url =
      "https://firestore.googleapis.com/v1/projects/plane-alert-800ff/databases/(default)/documents/aircraft-snapshots/52.46_13.52_100";
