...
// Adapted from https://github.com/flightaware/dump1090/blob/master/public_html/planeObject.js
// and https://github.com/wiedehopf/tar1090/blob/master/html/markers.js

// Define interfaces for icon data
...
// SVG Paths (copied from the prompt)
const _generic_plane_svg =
...
const CategoryIcons: { [key: string]: SvgIconData } = {
  A1: {
    key: 'A1',
    scale: 0.3,
    size: [64, 64],
  ...
  A7: {
    ...
    scale: 0.5,
    size: [22, 32],
    anchor: [11, 16], // Added missing anchor
...
// Type-specific icons
const TypeIcons: { [key: string]: SvgIconData } = {
  // Airbus A320 Family
  A318: _a320,
  A319: _a320,
  A320: _a320,
  ...
  B738: _b738,
  B739: _b738,
  B38M: _b738, // 737 MAX 8
  B39M: _b738, // 737 MAX 9

  // Boeing Wide-bodies (Approximation)
  B752: _b777, // 757-200
  B753: _b777, // 757-300
  B762: _b777, // 767-200
  B763: _b777, // 767-300
  B764: _b777, // 767-400
  B772: _b777, // 777-200
  B77W: _b777, // 777-300ER
  B773: _b777, // 777-300
  B77L: _b777, // 777-200LR
  B788: _b777, // 787-8
  B789: _b777, // 787-9
  B78X: _b777, // 787-10 (B78X is common)
  ...
  // Bombardier CRJ Series (using G650 as approximation)
  CRJ1: _g650, // CRJ-100
  CRJ2: _g650, // CRJ-200
  CRJ7: _g650, // CRJ-700
  CRJ9: _g650, // CRJ-900 (CL-600-2D24)
  CRJX: _g650, // CRJ-1000

  // Bombardier Challenger / Global (Business Jets)
  CL30: _g650, // Challenger 300 (BD-100-1A10)
  CL35: _g650, // Challenger 350 (BD-100-1A10)
  CL60: _g650, // Challenger 600 series
  CL65: _g650, // Challenger 650
  GLEX: _g650, // Global Express (BD-700-1A10)
  GL5T: _g650, // Global 5000 (BD-700-1A11)
  GL6T: _g650, // Global 6000 (BD-700-1A11)
  GL7T: _g650, // Global 7500 (BD-700-2A12)

  // Bombardier CSeries / Airbus A220 (Using A320 icon)
  BCS1: _a320, // A220-100 (BD-500-1A10)
  BCS3: _a320, // A220-300 (BD-500-1A11)

  // Embraer E-Jets (Using A320 icon)
  E170: _a320, // ERJ 170
  E175: _a320, // ERJ 175 (E75S, E75L)
  E190: _a320, // ERJ 190
  E195: _a320, // ERJ 195

  // Embraer Business Jets (Using G650 icon)
  E50P: _g650, // Phenom 100
  E55P: _g650, // Phenom 300 (EMB-505)
  E545: _g650, // Legacy 450
  E550: _g650, // Legacy 500

  // Gulfstream (Using G650 icon)
  GLF4: _g650, // G-IV / G450
  GLF5: _g650, // G-V / G550 (GV-SP)
  GLF6: _g650, // G650 / G650ER

  // Cessna Citation (Using G650 icon)
  C510: _g650, // Citation Mustang
  C525: _g650, // CitationJet/M2 (525)
  C25A: _g650, // CitationJet CJ2 (525A)
  C25B: _g650, // CitationJet CJ3 (525B)
  C25C: _g650, // CitationJet CJ4 (525C)
  C56X: _g650, // Citation Excel/XLS(+)
  C680: _g650, // Citation Sovereign
  C68A: _g650, // Citation Latitude (680A)
  C700: _g650, // Citation Longitude

  // Beechcraft King Air / Super King Air
  BE9L: _b200, // King Air C90
  BE20: _b200, // King Air 200/250
  BE30: _b200, // King Air 300/350

  // Light Aircraft (Using A1 icon)
  C172: CategoryIcons['A1'], // Cessna 172 (172P, 172S, 172N)
  C182: CategoryIcons['A1'], // Cessna 182
  C206: CategoryIcons['A1'], // Cessna 206
  PA28: CategoryIcons['A1'], // Piper PA-28 Cherokee/Archer (PA-28-181)
  PA32: CategoryIcons['A1'], // Piper PA-32 Cherokee Six/Saratoga
  PA44: CategoryIcons['A1'], // Piper PA-44 Seminole
  SR20: CategoryIcons['A1'], // Cirrus SR20
  SR22: CategoryIcons['A1'], // Cirrus SR22
  DA40: CategoryIcons['A1'], // Diamond DA40
  DA42: CategoryIcons['A1'], // Diamond DA42

  // Helicopters (Using A7 icon)
  B407: CategoryIcons['A7'], // Bell 407
  B06: CategoryIcons['A7'], // Bell 206 JetRanger (incl. 206L-3 LongRanger)
  B429: CategoryIcons['A7'], // Bell 429
  AS50: CategoryIcons['A7'], // Eurocopter AS350 Écureuil
  EC20: CategoryIcons['A7'], // Eurocopter EC120 Colibri
  EC30: CategoryIcons['A7'], // Eurocopter EC130
  EC35: CategoryIcons['A7'], // Eurocopter EC135
  EC45: CategoryIcons['A7'], // Eurocopter EC145
  R44: CategoryIcons['A7'], // Robinson R44
  R66: CategoryIcons['A7'], // Robinson R66
  S76: CategoryIcons['A7'], // Sikorsky S-76 (S-76C)

  // Military (Using C130 icon as generic military)
  C130: _c130,
  C30J: _c130,
  // Add other common military types if needed, mapping to _c130 or another icon
}; // Added missing closing brace

// Default icon
const DefaultIcon: SvgIconData = {
  key: 'default',
  scale: 0.73, // Adjusted scale to match heavy
  size: [64, 64],
  anchor: [32, 32], // Adjusted anchor to match heavy
  path: _heavy_svg, // Changed from _generic_plane_svg
};

/**
 * Selects the base SVG icon data based on aircraft type and category.
 * Prioritizes exact type match, then partial type match, then category match.
 * @param category Aircraft category (e.g., 'A1', 'A5', 'A7')
 * @param type Aircraft type (e.g., 'A320', 'B738', 'Boeing 737-800')
 * @returns The SvgIconData object for the selected icon.
 */
export function getBaseMarkerIconData(
  category: string | undefined,
  type: string | undefined // Corresponds to plane.model
): SvgIconData {
  console.log('getBaseMarkerIconData - Processing model:', type); // Added log
  // 1. Exact type match
  if (type && type in TypeIcons) {
    console.log(
      `getBaseMarkerIconData - Using exact type match: ${TypeIcons[type].key}`
    ); // Added log
    return TypeIcons[type];
  }

  // 2. Partial type match (check if model string contains a known type key)
  if (type) {
    const typeLower = type.toLowerCase();
    // Sort keys by length descending to match more specific types first (e.g., "B738" before "B73")
    const sortedTypeKeys = Object.keys(TypeIcons).sort(
      (a, b) => b.length - a.length
    );
    for (const key of sortedTypeKeys) {
      // Check if the model string *contains* the key (case-insensitive)
      if (typeLower.includes(key.toLowerCase())) {
        console.log(
          `getBaseMarkerIconData - Using partial type match: ${TypeIcons[key].key}`
        ); // Added log
  ...
  if (category && category in CategoryIcons) {
    console.log(
      `getBaseMarkerIconData - Using category match: ${CategoryIcons[category].key}`
    ); // Added log
    return CategoryIcons[category];
  }

  // 4. Default icon
  console.log(`getBaseMarkerIconData - Using default icon`); // Added log
  return DefaultIcon;
}

/**
 * Generates an SVG string from a path and size.
 * @param path The SVG path data.
 * @param size The [width, height] of the SVG.
 * @param fill The fill color (optional).
 * @param stroke The stroke color (optional).
 * @param strokeWidth The stroke width (optional).
 * @returns The complete SVG string.
 */
export function svgPathToSvg(
  path: string,
  size: [number, number],
  fill?: string | null,
  stroke?: string | null,
  ...
  let svg = `<svg width="${size[0]}px" height="${size[1]}px" viewBox="0 0 ${size[0]} ${size[1]}" version="1.1" xmlns="http://www.w3.org/2000/svg">`;
  svg += `<path d="${path}"`;
  if (stroke) {
    svg += ` stroke="${stroke}"`;
  }
  ...
  if (fill) {
    ...
    svg += ` fill="currentColor"`; // Default to currentColor to inherit from CSS
  }
  svg += '/></svg>';
  return svg;
}
...
export function svgPathToURI(
  ...
  const svgString = svgPathToSvg(path, size, fill, stroke, strokeWidth);
  // Use btoa for Base64 encoding (available in browser environments)
  return 'data:image/svg+xml;base64,' + btoa(svgString);
}

/**
 * Determines if the aircraft is a helicopter based on category or model name.
 * @param category Aircraft category.
 * @param model Aircraft model name.
 * @param isCustomHelicopter Flag from custom helicopter list.
 * @returns True if it's a helicopter, false otherwise.
 */
export function isHelicopter(
  category: string | undefined,
  model: string | undefined,
  isCustomHelicopter: boolean
): boolean {
  if (category === 'A7') {
    return true;
  }
  if (isCustomHelicopter) {
    return true;
  }
  if (model) {
