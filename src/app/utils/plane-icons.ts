  ...
  const engineMappings = [
    ...
    },
    {
      ...
      patterns: [
        ...
        'a330', // Airbus A330
        'a340', // Airbus A340 (Note: A340-200/300 are 4-engine, A340-500/600 are 4-engine. This might need a more specific rule if A340 icon should be quad.)
        'a350', // Airbus A350
        'b737',
        'b738',
        'b736',
  ...
  for (const { engines, patterns } of engineMappings) {
    if (patterns.some((p) => m.includes(p))) {
      if (ENGINE_ICON_INFO_MAP[engines]) {
        return ENGINE_ICON_INFO_MAP[engines];
      }
      break;
    }
  }
  // Default to twin-engine icon
  return { path: TWIN_ENGINE_SVG, iconType: 'twin_engine' };
}
