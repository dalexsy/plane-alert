...
/**
 * Heuristic thresholds for inferring aircraft type when model is unknown:
 * - Jets typically cruise above 8000m (FL260+) at speeds >= 250 m/s (~900 km/h)
 * - Turboprops max out around 200 m/s (~720 km/h) and cruise below 8000m
 * - Props/light aircraft rarely exceed 150 m/s and stay under 6000m
 */
const JET_ALTITUDE_THRESHOLD_M = 8000; // ~26,200 ft
const JET_VELOCITY_THRESHOLD_MS = 220; // ~790 km/h - slightly below 900 to catch climbing/descending jets
...
export function getIconPathForModel(
  ...
  if (categoryCode && /^C[0-7]$/i.test(categoryCode)) {
    return { path: GROUND_VEHICLE_SVG, iconType: 'ground_vehicle' };
  }
  ...
  if (TYPE_ICON_MAP[typeCode]) {
    ...
    if (m.includes('a400m')) {
      ...
      // If no model match found, use heuristic based on altitude + velocity
      if (!result) {
        // High-altitude + high-speed = almost certainly a jet
        ...
        if (
          ...
          // Low and slow = likely a light prop
          result = { path: SINGLE_ENGINE_SVG, iconType: 'single_engine' };
        } else {
          // Default fallback
          result = { path: TWIN_ENGINE_SVG, iconType: 'twin_engine' };
