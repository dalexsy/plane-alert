...
describe('RainService', () => {
  ...
  it('should initialize with default configuration', (done) => {
    service.getConfiguration().pipe(take(1)).subscribe((config) => {
  ...
  it('should not be raining initially', (done) => {
    service.getIsRaining().pipe(take(1)).subscribe((isRaining) => {
  ...
  it('should calculate correct intensity for different rain types', () => {
    ...
    expect(service.getIntensityForDescription('moderate rain')).toBeCloseTo(0.7, 1);
    expect(service.getIntensityForDescription('heavy rain')).toBeGreaterThan(0.8);
  ...
  it('should generate rain drops when starting rain', (done) => {
    service.startRain({ dropCount: 50 });
      service.getRainDrops().pipe(take(1)).subscribe((drops) => {
  ...
  it('should handle edge cases in weather conditions', () => {
    ...
    expect(() => service.updateWeatherConditions('Rain', 'rain', -1, 400)).not.toThrow();
  });
