...
import { updateWindowViewPlanes } from './plane-log/plane-log-window.util';
...
export class PlaneLogService {
  ...
  initialize(resultsOverlayComponent: ResultsOverlayComponent, windowViewOverlayComponent: WindowViewOverlayComponent): void {
  ...
  updatePlaneLog(planes: PlaneModel[]): PlaneModel[] {
    ...
    this.mapComponent.airportPlaneLog = visiblePlanes.filter((p) => p.airportCode != null) as unknown as PlaneLogEntry[];
    ...
    this.mapComponent.seenPlaneLog = [...militaryPlanes, ...otherPlanes] as unknown as PlaneLogEntry[];
