...
export const weatherCeilingProxy = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!stationRes.ok) {
        res.json({ feet: null, stationCode: null, kind: 'unavailable' as CeilingKind });
        return;
      }
      ...
      const metarResults = await Promise.all(
        candidates.map(async (candidate) => {
          ...
          try {
            const metarUrl =
              `https://aviationweather.gov/api/data/metar?format=json&ids=${stationIcao}`;
            ...
            const metar = Array.isArray(metars) && metars.length > 0 ? metars[0] : null;
            if (!metar) {
              return null;
            }
