...
exports.isJunkAircraftIdentityToken = isJunkAircraftIdentityToken;
exports.isHelicopterCategory = isHelicopterCategory;
exports.isHelicopterTypeCode = isHelicopterTypeCode;
exports.isHelicopterCallsign = isHelicopterCallsign;
exports.hasMeaningfulAircraftModel = hasMeaningfulAircraftModel;
exports.isLikelyHelicopter = isLikelyHelicopter;
function normalizeCallsignLocal(value) {
    if (!value)
        return '';
    return value.replace(/[^A-Z0-9]/gi, '').toUpperCase();
}
/** ADS-B filler tokens that look like a type/model but are not a real aircraft. */
function isJunkAircraftIdentityToken(value) {
    ...
    if (!v)
        return true;
    return /^(MLAT|ADSB|ADSBICAO|ADSBICAONT|UNKNOWN|NONE|NA|N\/A|HELICOPTER|OTHER|NULL|UNDEFINED)$/.test(v);
}
...
function isHelicopterTypeCode(icaoType) {
    if (!icaoType || isJunkAircraftIdentityToken(icaoType))
        return false;
    const t = icaoType.trim().toUpperCase().replace(/[-\s]/g, '');
    if (!t)
        return false;
    if (/^(EC|AS|BK)(\d{2}|\d{3})$/.test(t))
        return true;
...
function isHelicopterCallsign(callsign) {
    if (!callsign)
        return false;
 ...
 */
function hasMeaningfulAircraftModel(plane) {
    ...
    if (!desc)
        return false;
    if (isJunkAircraftIdentityToken(desc))
        return false;
    if (/^helicopter$/i.test(desc))
        return false;
    return true;
}
function isLikelyHelicopter(plane) {
    if (isHelicopterCategory(plane.category))
        return true;
    if (isHelicopterTypeCode(plane.t || plane.type))
        return true;
    if (isHelicopterCallsign(plane.flight || plane.callsign))
        return true;
    if (/^helicopter$/i.test((plane.desc || '').trim()))
        return true;
    return false;
}
//# sourceMappingURL=military-helicopter.js.map