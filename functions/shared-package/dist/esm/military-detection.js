 ...
 */
const INTERESTING_MIL_DESC_PATTERN = /\bF-?\d{2}\b|\bF\d{2}\b|\bE-?\d{1,2}\b|\bC-?\s*130\b|\bC-?\s*17\b|\bC-?\s*5\b|\bB-?\d{1,2}\b|\bA-?10\b|\bKC-?\d{2,3}\b|\bP-?\d\b|\bCH-?\d{2}\b|\bUH-?\d{2}\b|\bAH-?\d{2}\b|HERCULES|GLOBEMASTER|CHINOOK|APACHE|BLACK\s*HAWK|TYPHOON|TORNADO|RAFALE|GRIPEN|HORNET|EAGLE|RAPTOR|LIGHTNING|THUNDERBOLT|POSEIDON|STRATOFORT|ORION|AWACS|SENTRY|MRTT|TANKER|REFUEL/i;
/**
 ...
 */
export function shouldSkipBoringMilitaryFilter(plane) {
 ...
 */
export function isBoringMilitaryAircraft(plane) {
    ...
    if (!desc) {
        ...
        // a boring transport; callsign-only without type/desc is too (USAF/RCH VIP jets).
    ...
    if (BORING_AIRCRAFT_TYPES.some((boring) => {
