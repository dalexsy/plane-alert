...
async function checkAircraft(lat, lon, radiusKm) {
  ...
  try {
    ...
    console.log(
      `✈️  Military aircraft (mil=true): ${militaryAircraft.length}\n`
    );
    ...
    if (militaryAircraft.length > 0) {
      ...
      for (const plane of militaryAircraft) {
        ...
        const isBoring = BORING_AIRCRAFT_TYPES.includes(
        ...
        console.log(
          `  Type: ${aircraftType} ${
            isBoring ? "⚠️  FILTERED (boring)" : "✅ INTERESTING"
          }`
        );
      ...
      console.log(
        `⚠️  Filtered out (boring): ${
          militaryAircraft.length - interesting.length
        }`
      );
    } else {
