...
async function checkDevices() {
  try {
    ...
    for (const [docId, data] of devices.entries()) {
      ...
      console.log(`  Home Location: ${data.home ? `${data.home.lat}, ${data.home.lon}` : 'not set'}`);
      ...
      console.log(`  Proximity Alerts: ${data.notifyProximity ? 'enabled' : 'disabled'}`);
      console.log(`  Created: ${data.createdAt ? new Date(data.createdAt._seconds * 1000).toISOString() : 'unknown'}`);
      console.log(`  Updated: ${data.updatedAt ? new Date(data.updatedAt._seconds * 1000).toISOString() : 'unknown'}`);
      console.log('');
    }
